...
Setup and execution
Open src/automatic_street_light.circ in classic Logisim 2.7.1. Newer Logisim-evolution versions may import it, but compatibility has not been tested.
Select the Poke tool and click input pins D, M and E to change their values.
Follow all eight rows in data/expected_truth_table.csv.
Compare output pin L and the LED with the expected output.
Fill results/simulation_observations.csv and save real screenshots in results/.
Open docs/Automatic_Street_Light_Control.pptx, replace identity placeholders, and update the results slide only after simulation.
